...
statparams = cbind(rep(alpha = 0.9, length(beta)), beta = c(-0.99, -0.9, 0, 0.9, 0.99))
# define explosive models
expparams  = cbind(rep(alpha = 1, length(beta)), beta = c(-0.99, -0.9, 0, 0.9, 0.99))
#define lag vector
pvec         = as.matrix(3:11)
...
ADFSimtest = function(alpha, beta){
  ...
  for(iCounterSim in 1:iSimulations){
    x = simprocess(alpha, beta, iSample)
...
# create latex table
colnames(teststat) = sapply(beta, as.character)
rownames(testexp[c(1, 9), ]) = sapply(c(3,11), as.character)
