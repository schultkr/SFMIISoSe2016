...
Description: 'Simulates Augmented Dickey-Fuller tests for stationary and non-stationary ARMA processes. The simulated process x(t) is
x(t) = alpha x(t-1) + beta epsilon(t-1) + epsilon(t),
where the error term is a Gaussian White Noise process. Each process is
simulated 1000 times. For each simulated process the number of lags 
included for the ADF test varies between 3 to 11.'
...
Output: 'A plot for the level (alpha = 0.9) and the power (alpha = 1)
of Augmented Dickey-Fuller tests. The rejection probability for beta
= -0.99 (black), beta = -0.90 (red), beta = 0 (blue), beta = -0.90 
(green) and beta = -0.99 (magenta) depending on the number of included
lags are displayed. Two tables are printed for the minimum and maximum
number of lags.'
